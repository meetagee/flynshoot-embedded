...
#define	ZB_COODINATOR_DISABLE_PERMIT_JOIN	0x00
...
public:
	...
	uint8_t update();
	uint8_t update_irq(uint8_t);

	int zwrite(uint8_t*, uint32_t);
	int zread(uint8_t*, uint32_t);
	void zdisable_rev_async();
	void zenable_rev_async();
	...
	uint8_t znp_frame_parser(uint8_t data);

private:
#define MAX_LEN_ZNP		256
