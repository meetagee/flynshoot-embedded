...
#if (ZB_ZCL_EN == 1)
#define ZB_ZCL(fmt, ...)		xprintf(fmt, "ZB_ZCL", ##__VA_ARGS__)
#else
#define ZB_ZCL(fmt, ...)
#endif
...
zclProcMsgStatus_t zcl_ProcessMessageMSG(af_incoming_msg_t *pkt) {
	...
	if (zcl_ProfileCmd(inMsg.hdr.fc.type)) {
		...
		case ZCL_CMD_READ_RSP: {
			ZB_ZCL("[zcl_ProcessMessageMSG] ZCL_CMD_READ_RSP\n");
			...
			if (inMsg.attrCmd != NULL) {
				...
				for (int i = 0; i < readRspCmd->numAttr; i++) {
					zclReadRspStatus_t *statusRec = &(readRspCmd->attrList[i]);
					if (statusRec->status == ZCL_STATUS_SUCCESS) {
						zclOutgoingMsg_t* pOutgoingMsg = (zclOutgoingMsg_t*)malloc(sizeof(zclOutgoingMsg_t));
						...
						pOutgoingMsg->data = (uint8_t*)malloc(statusRec->dataLen);
						memcpy(pOutgoingMsg->data, statusRec->data, statusRec->dataLen);

						task_post_common_msg(AC_TASK_ZIGBEE_ID, AC_ZIGBEE_ZCL_CMD_HANDLER, (uint8_t*)&pOutgoingMsg, sizeof(pOutgoingMsg));
					} else {
						ZB_ZCL("statusRec->status == ZCL_STATUS_FAILURE\n");
					}
				}
			}
		...
		case ZCL_CMD_REPORT: {
			ZB_ZCL("[zcl_ProcessMessageMSG] ZCL_CMD_REPORT\n");
			zclParseCmd_t parseCmd;
			parseCmd.endpoint = pkt->dst_endpoint;
			parseCmd.dataLen = inMsg.pDataLen;
			parseCmd.pData = inMsg.pData;
			...
			if (inMsg.attrCmd != NULL) {
				...
				for (int i = 0; i < reportCmd->numAttr; i++) {
					...
					pOutgoingMsg->data = (uint8_t*)malloc(reportRec->dataLen);
		...
		switch (inMsg.hdr.commandID) {
		case ZCL_CMD_READ: {
			ZB_ZCL("[zcl_ProcessMessageMSG] ZCL_CMD_READ\n");
			if (pkt->cluster_id == ZCL_CLUSTER_ID_SS_IAS_ZONE) {
				if (inMsg.pDataLen == 6) {
					...
					pOutgoingMsg->data = (uint8_t*)malloc(inMsg.pDataLen);
